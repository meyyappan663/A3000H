...
- Swap the placeholder `ic_launcher` drawable for a real designed icon if desired
  (a simple generated one is included now so the project builds).
...
2. Let Gradle sync (it uses Android Gradle Plugin 8.6.1 and JDK 17 — see the root
   `build.gradle` and `.github/workflows/build.yml`).
3. Optionally replace `app/src/main/res/drawable/ic_launcher.xml` with a custom icon.
